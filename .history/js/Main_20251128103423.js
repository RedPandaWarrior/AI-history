...
const userMsgPanel = {
    ...
    const updatePos = () => {
        ...
        this.root.style.left = `${rect.right}px`;
        this.root.style.top = `${rect.top + rect.height / 2}px`;
        this._rafId = requestAnimationFrame(updatePos);
    };

    // 可以配合 CSS transition 做一点点缓动
    this.root.style.transition = 'left 0.1s linear, top 0.1s linear';
